...
# ─── פאנל פונדמנטלי (lazy — טוען רק בפתיחה) ──────────────────────────────────
def _fmt_cap(v):
    if v is None: return "—"
...
with st.expander("📊 Fundamental Profile — לחץ לפרטים", expanded=False):
    fund_data = _load_fundamentals(ticker)
    if not fund_data or fund_data.get("error"):
        st.caption(f"⚠️ Fundamentals unavailable{': ' + fund_data['error'] if fund_data else ''}")
    else:
        fd = fund_data
